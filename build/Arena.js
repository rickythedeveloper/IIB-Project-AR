...
export default class Arena {
     ...
     */
    positionFromCameraToDominant(position) {
    ...
    _mainLoop() {
        ...
        for (let markerIndex = 0; markerIndex < this.markers.length; markerIndex++) {
            if (!this.markers[markerIndex].visible)
                continue;
            if (nearestMarkerIndex === undefined)
                nearestMarkerIndex = markerIndex;
            else if (this.markers[markerIndex].position.length() < this.markers[nearestMarkerIndex].position.length())
                nearestMarkerIndex = markerIndex;
        }
        if (nearestMarkerIndex === undefined)
            return;
