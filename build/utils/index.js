    ...
    const recordValueInterval = setInterval(() => {
        for (let i = 0; i < markers.length; i++) {
            for (let j = 0; j < markers.length; j++) {
                if (i === j)
                    continue;
                if (!markers[i].visible || !markers[j].visible)
                    continue;
                ...
                if (p010.equals(zeroVector) || p020.equals(zeroVector))
                    continue;
                if (p010.length() > maxMarkerDistance || p020.length() > maxMarkerDistance)
                    continue;
    ...
    const setValueInterval = setInterval(() => {
        ...
        for (let i = 0; i < markers.length; i++) {
            for (let j = 0; j < markers.length; j++) {
                ...
                const confidence = Math.min(1, positions.length / minMeasurements) *
                ...
                if (confidence === 1)
                    markerPositionsMatrix[i][j] = meanPosition;
